...
suite("subagent model guidance", () => {
	test("explains native model inheritance and preferred routing profiles", () => {
		...
		assert.ok(guidance.includes("Without runSubagent.model, the general-purpose subagent inherits the parent model and system prompt"));
		assert.ok(guidance.includes("agentName selects behaviour/custom instructions independently of model"));
		assert.ok(guidance.includes("set runSubagent.model to the exact model-picker string"));
